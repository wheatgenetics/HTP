...
#
# Added capability to handle cases where number of autopilot log CAM events is greater thant number of image files,
# by matching closest CAM event time to Image EXIF time.
#
...
def get_image_log_data(ffilename,fcamEvents,fcamIndex):
    ...
    return fsampledate,fsampletime,fuas_latitude,fuas_longitude,fuas_position_x,fuas_position_y,fuas_position_z, fuas_latzone, fuas_longzone,fuas_altitude_ref,ff_checksum
...
print "Number of Image Files Found", len(imagefiles)
print "Number of CAM events Found", len(camEvents)
...
camIndex = 0
matchIndex=0
#previous_time_delta = calendar.timegm(time.gmtime())
...
for f in imagefiles:
    ...
    if len(imagefiles) == len(camEvents):
        ...
        uas_position_z,uas_latzone, uas_longzone,uas_altitude_ref,f_checksum=get_image_log_data(filename,camEvents,camIndex)
    ...
    # Number of Autopilot Log CAM events > Number of Image files

        matched = False
 #       matchIndex = camIndex
        camSampleDateTime = cam_sample_date_utc + ' ' + cam_sample_time_utc
        cam_time = time.mktime(time.strptime (camSampleDateTime,"%Y/%m/%d %H:%M:%S"))

        while not matched:
            uas_sample_date_utc,uas_sample_time_utc,uas_latitude,uas_longitude,uas_position_x,uas_position_y,\
            uas_position_z,uas_latzone, uas_longzone,uas_altitude_ref,\
            f_checksum=get_image_log_data(filename,camEvents,matchIndex)

            uasSampleDateTime = uas_sample_date_utc + ' ' + uas_sample_time_utc
            uas_time = time.mktime(time.strptime (uasSampleDateTime,"%Y/%m/%d %H:%M:%S"))
            time_delta = abs(uas_time - cam_time)
            ...
            if time_delta > previous_time_delta:
                ...
                metadata_record[13]=uas_altitude_ref
                matchIndex+=1
            print "Time Difference ", camIndex, matchIndex, time_delta,previous_time_delta,matched
            previous_time_delta = time_delta

    time.sleep(0.1)
    print metadata_record[9], metadata_record[10], metadata_record[19], metadata_record[20]
