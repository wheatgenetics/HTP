...
def get_gps_log_info(fflightLog):
    gpsEventDict = {}
    gpsEventList = []
    print "Reading apm log", fflightLog
    rowCount=0
    timeInSecs=0.0
    prevTime=0.0
    nextTime=0.0
    ...
    with open(fflightLog, 'rU') as logfile:
        ...
        for row in log:
            ...
            if rowCount > 1:
                ...
                gpsUTC=datetime.datetime.strptime(rowFields[11],"%Y-%m-%d %H:%M:%S.%f")
                epoch=datetime.datetime.fromtimestamp(0)
                timeInSecs=(gpsUTC-epoch).total_seconds()
                ...
                nextAlt=altitude
                if takingVideo=='1':
                    gpsEventList=[latitude,longitude,altitude,takingVideo,interpolated,timeInSecs]
                    gpsEventDict[timeInSecs] = gpsEventList
                #print gpsEventList
                if rowCount >2 and takingVideo=='1':
                    #print rowFields[11],prevTime,nextTime,prevLat,nextLat,prevLong,nextLong
                    timeDelta=nextTime-prevTime
                    latDelta=nextLat-prevLat
                    longDelta=nextLong-prevLong
                    altDelta=nextAlt-prevAlt
                    #print '$',latDelta,longDelta,altDelta
                    newTime=prevTime +0.01
                    newLat=prevLat
                    newLong=prevLong
                    newAlt=prevAlt
                    if abs(timeDelta) > 0.01:
                        while newTime < nextTime:
                            inc=timeDelta/0.01
                            #print '*',inc,timeDelta,newTime,latDelta,longDelta,altDelta,newAlt
                            if latDelta > 0.0:
                            ...
                            if altDelta > 0.0:
                                ...
                                newAlt -= abs(altDelta) / inc
                            gpsEventList=[newLat,newLong,newAlt,takingVideo,True,newTime]
