            ...
            if takingVideo == '1':
                ...
                if abs(timeDelta) > 10:
                    ...
                    while newTime < timestamp:
                        ...
                        gpsEventDict[lastTime] = gpsEventList
                        lastTime = newTime
                        newTime += 10
                        #print "Stored 2:", gpsEventList
...
for uasPath in uasFolderList:
    ...
    imagefiles = get_image_file_list(uasPath,imageType,imageFileList)
       ...
       #timeOffset=get_imageTime_logTime_delta(firstImage,firstLogEntry)
    #print "Time difference between EXIF and Log for first Image (ms):",timeOffset
    ...
    try:
        ...
        #Get Date, Time and Frame Number From Image File EXIF metadata
            ...
            with open(imagefilepath,'rb') as image:
                cam_time_in_seconds,cam_frame_number=get_image_exif_data(image,frameRate)
            image.close()
            timestamp = cam_time_in_seconds
            #timestamp=cam_time_in_seconds - timeOffset
            #print int(cam_time_in_seconds),int(timestamp)
        ...
        # Find the time index in GPS events that is less than or equal to the frame time
            ...
            logTimeIndex=0
            #if timeOffset > 0:
            #    logTimeIndex = bisect.bisect_left(sortedKeys, timestamp + 5)
            #else:
