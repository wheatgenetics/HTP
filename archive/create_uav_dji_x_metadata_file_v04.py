...
#
# Version 0.4 May 25 Added capability to compute plot intersections for each range and store in file.
#                    Added capbility to define output path for output files instead of using input path.
#
...
#
    ...
    startLon=metadatalist[0][8]
    startLat=metadatalist[0][7]
    endLon=metadatalist[-1][8]
    endLat = metadatalist[-1][7]
    pointA = Point(startLon,startLat)
    pointB = Point(endLon, endLat)
    rangeLineSegment = wkt.loads(LineString([pointA, pointB]).wkt)
    rangeLineSegments[image_set]=rangeLineSegment
