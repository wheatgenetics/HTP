#!/usr/bin/python
#
# Program: create_uav_dji_x5_metadata_file
#
# Version: 0.9 May 1,2017 Added support for renaming image files.
#
# Version: 0.8 April 28,2017 Added support for processing 1-n image folders.
#
# Version: 0.7 October 29,2016 Added support for improved time accuracy using framenumber at 24 frames/second.
#                              Simplified logic in interpolate_time function and removed errors.
#
# Version: 0.6 October 26,2016 Added timezonefinder library to determine GPS timezone from lat/long position.
#
# Version: 0.5 June 16,2016 Switched to piexif library to incorporate EXIF write capabilities
#
...
#**********************************************
with open('/Users/mlucas/Desktop/gpsEvents.csv', 'wb') as csvfile:
...
with open('/Users/mlucas/Desktop/gpsEvents.csv', 'ab') as csvfile:
...
for uasPath in uasFolderList:
    ...
    startPos = uasPath.find('C0')
    image_set = uasPath[startPos:startPos + 4]
    print ' '
...
#
    uasMetadataFile= uasPath + flightId + '_' + image_set +'_metadata.csv'
    print ''
    ...
    try:
        ...
        # Find the time index in GPS events that is less than or equal to the frame time
            logTimeIndex = bisect.bisect_right(sortedKeys, cam_time_in_seconds + 5.0)
            ...
            imageParts = f_jpg.split('_')
            imageID = imageParts[-1]
            imageHeader=''
            y=len(imageParts)-1
            for x in range(0,y):
                imageHeader+=imageParts[x] + '_'
            newimagefilepath = uasPath + imageHeader+imgTime+'_'+imageID
            newimagefilename = imageHeader+imageID
            oldimagefilepath = uasPath+f_jpg
            ...
            referenceAltitude=317.1 # Altitude of Ashland in Meters
            altRef=0 # Above Sea Level
            altMeters=uas_position_z+referenceAltitude
            try:
                with open(filename_jpg, 'rb') as image:
                 #   print "Writing EXIF data for ", filename_jpg
                 #   print ''
                    exif_dict=piexif.load(filename_jpg)
                    ...
                    piexif.insert(exif_bytes,filename_jpg)
                image.close()
            except:
                pass
            camIndex += 1
    except Exception, e:
        print '*** Error*** Unable to process image file ',filename_dng, " or ",filename_jpg
        print '*** Error Code:', e
