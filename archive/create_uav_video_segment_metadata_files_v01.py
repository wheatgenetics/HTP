...
#
# Version: 0.2 May 9,2017 Update to handle multiple image folders produced by video segmentation program.
#
...
cmdline.add_argument('-s','--segtimes',help='Segment Timing File Name')
...
with open(segTimePath, 'rb') as segTimes:
    timeReader = csv.reader(segTimes)
    for row in timeReader:
        segStart=row[1]
        segTimeList.append(segStart)
...
for uasPath in uasFolderList:
    ...
    #print ' '
    #print "Camera ID: ", sensor_id
    startPos = uasPath.find('WP')
    image_set = uasPath[startPos:startPos + 8]
    print ' '
...
#Initialize timestamp to the first timestamp in the log
    ...
    timestamp=sortedKeys[0] + 5 + int(segTimeList[segIndex])
    frameIndex = 0
    logTimeIndex= 0
