...
record_count = ("SELECT record_id FROM phemu_images")

db_load = "LOAD DATA LOCAL INFILE %s INTO TABLE phemu_images IGNORE 1 LINES (@camera_sn,@image_file_name," \
          ...
          "SET image_file_name=@image_file_name,camera_sn=@camera_sn," \
...
update_run_id = "UPDATE phemu_images SET run_id=%s where run_id IS NULL AND record_id > %s"

db_check = "SELECT record_id FROM phemu_images WHERE run_id LIKE %s"

fix_auto_increment = "ALTER TABLE phemu_images AUTO_INCREMENT=1"

print('Loading file', fname, 'into Database')

cursor.execute(record_count,)
startcount = cursor.rowcount
print("Number of records in phemu_images before insert is",startcount)
...
print("Number of records in phemu_images after insert is",endcount)
