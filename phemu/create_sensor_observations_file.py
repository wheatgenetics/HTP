#!/usr/bin/python
#
# Program: create_sensor_observations_file
#
# Takes a p3 sensor metadata file and generates output for the following new platform independent tables:
# 1. sensor_measurements
# 2. sensor_offsets
...
outputPath=dirPath+args.input[0:fref]+'p3pi.csv'
...
with open(outputPath, 'wb') as csvfile:
    ...
    header.writerow(
        ['run_id','sensor_id','time_utc','date_utc','position','northing','easting','altitude','utm_zone','observation',
         'gps1_utc','gps1_altitude','gps1_longitude','gps1_latitude','gps2_utc','gps2_altitude','gps2_longitude',
         'gps2_latitude'])
csvfile.close()
    ...
    with open(inputPath,'rU') as metadataFile:
        ...
        for row in metadata:
            ...
            if inputCount > 1:
                ...
                lineitem.writerow([run_id, sensorID, utcTime, utcDate,point_position,northing, easting, altitude, utmZone,observation,
